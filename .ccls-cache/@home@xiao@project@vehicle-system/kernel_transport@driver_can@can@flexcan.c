...
static struct flexcan_dma_ops *dma_ops;
...
int flexcan_dma_register_ops(const struct flexcan_dma_ops *ops)
{
    
    if (!ops || !ops->get_frame)
        return -EINVAL;  

    dma_ops = ops;
    return 1;  
}
EXPORT_SYMBOL(flexcan_dma_register_ops);

void flexcan_dma_unregister_ops(void)
{
    
    dma_ops = NULL;
}
EXPORT_SYMBOL(flexcan_dma_unregister_ops);
...
static void flexcan_read_fifo_dummy(struct net_device *dev, struct can_frame *cf)
{
    struct flexcan_priv *priv = netdev_priv(dev);
    struct flexcan_regs __iomem *regs = priv->base;
    
    // 最小化操作：只读取必要的寄存器清除标志
    u32 reg_ctrl = flexcan_read(&regs->ctrl);
    u32 reg_id = flexcan_read(&regs->mb[priv->rx_mb_idx].can_id);
    u32 reg_data = flexcan_read(&regs->mb[priv->rx_mb_idx].data[0]);
...
{
	if (!dma_ops->get_frame || !dma_ops->rx_data_complete) {
        netdev_err(dev, "DMA ops not initialized\n");
        return 0;
    }
    ...
    // 尝试获取 DMA 帧
    struct can_frame *cf = dma_ops->get_frame(&index);
    
    if (unlikely(!cf)) {
        // 缓冲区满，丢弃帧但必须读取硬件 FIFO
        struct can_frame dummy;
        flexcan_read_fifo_dummy(dev, &dummy);
        
        stats->rx_dropped++;
        
        // 限制警告频率
        if (!warn_once) {
            netdev_warn(dev, "RX ring full, dropping frames\n");
            warn_once = true;
        }
        
        return 1; // 返回 1 表示处理了一个帧（丢弃）
    }
    ...
    dma_ops->rx_data_complete(index);
