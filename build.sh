...
build_uboot() {
    ...
    make ARCH=arm CROSS_COMPILE="$TOOLCHAIN" mx6ull_14x14_evk_defconfig
...
build_kernel() {
    ...
    make ARCH=arm CROSS_COMPILE="$TOOLCHAIN" imx_v7_defconfig
    make ARCH=arm CROSS_COMPILE="$TOOLCHAIN" -j$(nproc) zImage dtbs
    mkdir -p "$OUTPUT_DIR/kernel"
    cp arch/arm/boot/zImage "$OUTPUT_DIR/kernel/"
    cp arch/arm/boot/dts/imx6ull-14x14-evk.dtb "$OUTPUT_DIR/kernel/"
    popd >/dev/null
